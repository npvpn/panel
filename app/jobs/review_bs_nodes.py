...
def _bot_limits(db):
    ...
    for bot_id, data in db.query(BotSettings.bot_id, BotSettings.data).all():
        ...
        limits[bot_id] = (settings.get("bs_daily_limit") or 0,
                          settings.get("bs_monthly_limit") or 0)
    return limits
...
def review_bs_nodes():
    ...
    with GetDB() as db:
        ...
        usage_rows = db.query(
            NodeUserBsUsage.user_id, NodeUserBsUsage.daily_used, NodeUserBsUsage.daily_period,
            NodeUserBsUsage.monthly_used, NodeUserBsUsage.monthly_period,
        ).filter(NodeUserBsUsage.node_id.in_(bs_node_ids)).all()
        ...
        user_bot = dict(db.query(User.id, User.bot_id).filter(User.id.in_(user_ids)).all()) \
            if user_ids else {}
        ...
        for node_id, user_id in to_block:
            ...
            if not dbuser:
                continue
            db.add(NodeUserBlock(node_id=node_id, user_id=user_id, period="agg",
