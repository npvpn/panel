...
def send_webhook(json_data, webhook):
    ...
    try:
        ...
        logger.debug(f"Discord payload delivered successfully, code {result.status_code}.")
...
def report_status_change(username: str, status: str, admin: Admin = None):
    ...
    send_webhooks(
        json_data=statusChange, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
...
**Has Next Plan:**{has_next_plan}""",
    ...
    send_webhooks(
        json_data=reportNewUser, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
...
**Has Next Plan:**{has_next_plan}""",
    ...
    send_webhooks(
        reportUserModification, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
...
def report_user_deletion(username: str, by: str, admin: Admin = None):
    ...
    send_webhooks(
        json_data=userDeletion, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
...
def report_user_usage_reset(username: str, by: str, admin: Admin = None):
    ...
    send_webhooks(
        json_data=userUsageReset, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
...
def report_user_subscription_revoked(username: str, by: str, admin: Admin = None):
    ...
    send_webhooks(
        json_data=subscriptionRevoked, admin_webhook=admin.discord_webhook if admin and admin.discord_webhook else None
    )
